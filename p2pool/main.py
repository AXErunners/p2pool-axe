...
import axe.p2p as axe_p2p, axe.data as axe_data
from axe import stratum, worker_interface, helper
...
class keypool():
    ...
    def paytotal(self):
        ...
        for i in xrange(len(pubkeys.keys)):
            self.payouttotal += node.get_current_txouts().get(axe_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)*1e-8
...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        ...
        def connect_p2p():
            ...
            print '''Testing axed P2P connection to '%s:%s'...''' % (args.axed_address, args.axed_p2p_port)
        ...
        url = '%s://%s:%i/' % ('https' if args.axed_rpc_ssl else 'http', args.axed_address, args.axed_rpc_port)
        print '''Testing axed RPC connection to '%s' with username '%s'...''' % (url, args.axed_rpc_username)
        axed = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(args.axed_rpc_username + ':' + args.axed_rpc_password)), timeout=30)
        yield helper.check(axed, net)
        temp_work = yield helper.getwork(axed, net)
        
        axed_getnetworkinfo_var = variable.Variable(None)
        @defer.inlineCallbacks
        def poll_warnings():
            axed_getnetworkinfo_var.set((yield deferral.retry('Error while calling getnetworkinfo:')(axed.rpc_getnetworkinfo)()))
        yield poll_warnings()
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            if address is not None:
                res = yield deferral.retry('Error validating cached address:', 5)(lambda: axed.rpc_validateaddress(address))()
            ...
            if address is None:
                ...
                address = yield deferral.retry('Error getting payout address from axed:', 5)(lambda: axed.rpc_getaccountaddress('p2pool'))()
            
            with open(address_path, 'wb') as f:
                f.write(address)
            
            my_pubkey_hash = axe_data.address_to_pubkey_hash(address, net.PARENT)
            print '    ...success! Payout address:', axe_data.pubkey_hash_to_address(my_pubkey_hash, net.PARENT)
            print
            ...
            for i in xrange(args.numaddresses):
                address = yield deferral.retry('Error getting a dynamic address from axed:', 5)(lambda: axed.rpc_getnewaddress('p2pool'))()
                new_pubkey = axe_data.address_to_pubkey_hash(address, net.PARENT)
            ...
            for i in xrange(len(pubkeys.keys)):
                print '    ...payout %d: %s' % (i, axe_data.pubkey_hash_to_address(pubkeys.keys[i], net.PARENT),)
        ...
        node = p2pool_node.Node(factory, axed, shares.values(), known_verified, net)
        yield node.start()
        ...
        wb = work.WorkerBridge(node, my_pubkey_hash, args.donation_percentage, merged_urls, args.worker_fee, args, pubkeys, axed)
        web_root = web.get_web_root(wb, datadir_path, axed_getnetworkinfo_var, static_dir=args.web_static)
                ...
                def signedOn(self):
                    ...
                    def new_share(share):
                        ...
                        if share.pow_hash <= share.header['bits'].target and abs(share.timestamp - time.time()) < 10*60:
                            ...
                            message = '\x02%s BLOCK FOUND by %s! %s%064x' % (net.NAME.upper(), axe_data.script2_to_address(share.new_script, net.PARENT), net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    my_shares_per_s = sum(datum['work']/dt/axe_data.target_to_average_attempts(datum['share_target']) for datum in datums)
                    ...
                    if height > 2:
                        ...
                        paytot = 0.0
                        for i in xrange(len(pubkeys.keys)):
                            curtot = node.get_current_txouts().get(axe_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)
                            paytot += curtot*1e-8
                        ...
                        this_str += '\n Pool: %sH/s Stale rate: %.1f%% Expected time to block: %s' % (
                            ...
                            100*stale_prop,
                            math.format_dt(2**256 / node.axed_work.value['bits'].target / real_att_s),
                        )
                        
                        for warning in p2pool_data.get_warnings(node.tracker, node.best_share_var.value, net, axed_getnetworkinfo_var.value, node.axed_work.value):
                            print >>sys.stderr, '#'*40
...
def run():
    ...
    parser.add_argument('--net',
        ...
        action='store', choices=sorted(realnets), default='axe', dest='net_name')
    ...
    parser.add_argument('-a', '--address',
        help='generate payouts to this address (default: <address requested from axed>), or (dynamic)',
        type=str, action='store', default=None, dest='address')
    parser.add_argument('-i', '--numaddresses',
        help='number of axe auto-generated addresses to maintain for getwork dynamic address allocation',
    ...
    worker_group.add_argument('-f', '--fee', metavar='FEE_PERCENTAGE',
        help='''charge workers mining to their own axe address (by setting their miner's username to a axe address) this percentage fee to mine on your p2pool instance. Amount displayed at http://127.0.0.1:WORKER_PORT/fee (default: 0)''',
        type=float, action='store', default=0, dest='worker_fee')
    
    axed_group = parser.add_argument_group('axed interface')
    axed_group.add_argument('--axed-config-path', metavar='AXED_CONFIG_PATH',
        help='custom configuration file path (when axed -conf option used)',
        type=str, action='store', default=None, dest='axed_config_path')
    axed_group.add_argument('--axed-address', metavar='AXED_ADDRESS',
        help='connect to this address (default: 127.0.0.1)',
        type=str, action='store', default='127.0.0.1', dest='axed_address')
    axed_group.add_argument('--axed-rpc-port', metavar='AXED_RPC_PORT',
        help='''connect to JSON-RPC interface at this port (default: %s <read from axe.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.RPC_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='axed_rpc_port')
    axed_group.add_argument('--axed-rpc-ssl',
        help='connect to JSON-RPC interface using SSL',
        action='store_true', default=False, dest='axed_rpc_ssl')
    axed_group.add_argument('--axed-p2p-port', metavar='AXED_P2P_PORT',
        help='''connect to P2P interface at this port (default: %s <read from axe.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.P2P_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='axed_p2p_port')
    
    axed_group.add_argument(metavar='AXED_RPCUSERPASS',
        help='axed RPC interface username, then password, space-separated (only one being provided will cause the username to default to being empty, and none will cause P2Pool to read them from axe.conf)',
        type=str, action='store', default=[], nargs='*', dest='axed_rpc_userpass')
    ...
    args.axed_rpc_username, args.axed_rpc_password = ([None, None] + args.axed_rpc_userpass)[-2:]
    
    if args.axed_rpc_password is None:
        conf_path = args.axed_config_path or net.PARENT.CONF_FILE_FUNC()
        if not os.path.exists(conf_path):
            parser.error('''axe configuration file not found. Manually enter your RPC password.\r\n'''
                '''If you actually haven't created a configuration file, you should create one at %s with the text:\r\n'''
                '''\r\n'''
                '''server=1\r\n'''
                '''rpcpassword=%x\r\n'''
                '''\r\n'''
                '''Keep that password secret! After creating the file, restart axe.''' % (conf_path, random.randrange(2**128)))
        ...
        if args.axed_rpc_password is None:
            parser.error('''axe configuration file didn't contain an rpcpassword= line! Add one!''')
    ...
    if args.address is not None and args.address != 'dynamic':
        try:
            args.pubkey_hash = axe_data.address_to_pubkey_hash(args.address, net.PARENT)
