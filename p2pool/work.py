...
import axe.getwork as axe_getwork, axe.data as axe_data
from axe import helper, script, worker_interface
...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def __init__(self, node, my_pubkey_hash, donation_percentage, merged_urls, worker_fee, args, pubkeys, axed):
        ...
        def compute_work():
            t = self.node.axed_work.value
            bb = self.node.best_block_header.value
            if bb is not None and bb['previous_block'] == t['previous_block'] and self.node.net.PARENT.POW_FUNC(axe_data.block_header_type.pack(bb)) <= t['bits'].target:
                ...
                t = dict(
                    ...
                    transactions=[],
                    transaction_fees=[],
                    merkle_link=axe_data.calculate_merkle_link([None], 0),
                    subsidy=self.node.axed_work.value['subsidy'],
                    last_update=self.node.axed_work.value['last_update'],
                    payment_amount=self.node.axed_work.value['payment_amount'],
                    packed_payments=self.node.axed_work.value['packed_payments'],
                )
                '''

            self.current_work.set(t)
        self.node.axed_work.changed.watch(lambda _: compute_work())
    ...
    def get_user_details(self, username):
        ...
        for symbol, parameter in zip(contents2[::2], contents2[1::2]):
            if symbol == '+':
                try:
                    desired_pseudoshare_target = axe_data.difficulty_to_target(float(parameter))
                except:
                    if p2pool.DEBUG:
                        log.err()
            elif symbol == '/':
        ...
        if random.uniform(0, 100) < self.worker_fee:
            ...
            try:
                pubkey_hash = axe_data.address_to_pubkey_hash(user, self.node.net.PARENT)
            except: # XXX blah
                if self.args.address != 'dynamic':
                    pubkey_hash = self.my_pubkey_hash
    ...
    def preprocess_request(self, user):
        ...
        if time.time() > self.current_work.value['last_update'] + 60:
            raise jsonrpc.Error_for_code(-12345)(u'lost contact with axed')
